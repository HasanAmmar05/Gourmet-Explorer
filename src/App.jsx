  ...
  const searchRecipes = async (page = 1) => {
    ...
    try {
      ...
      setRecipes((prevRecipes) =>
        page === 1 ? response.data.results : [...prevRecipes, ...response.data.results]
      )
  ...
  const saveRecipe = (recipe) => {
    setSavedRecipes((prev) => {
      if (prev.some((r) => r.id === recipe.id)) {
        showToast("Recipe Removed", `${recipe.name} has been removed from your saved recipes.`)
        return prev.filter((r) => r.id !== recipe.id)
      } else {
        showToast("Recipe Saved", `${recipe.name} has been added to your saved recipes.`)
        return [...prev, recipe]
      }
    })
  }
      ...
      <div className="bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-900 dark:to-gray-800 min-h-screen">
        <header className="sticky top-0 z-50 bg-white dark:bg-gray-800 shadow-md">
          <div className="container mx-auto px-4 py-4 flex justify-between items-center">
            <h1 className="text-4xl font-bold bg-gradient-to-r from-cyan-400 to-teal-400 bg-clip-text text-transparent"> Easy Recipe </h1>
            <div className="flex items-center space-x-4">
              <button
                ...
                aria-label={showSavedRecipes ? "Hide saved recipes" : "Show saved recipes"}
              >
          ...
          <AnimatePresence mode="wait">
            {showSavedRecipes ? (
              ...
              >
                ...
                <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-white">Saved Recipes</h2>
              ...
              >
                ...
                {loading && <p className="text-center mt-4">Loading more recipes...</p>}
              </motion.div>
            )}
          </AnimatePresence>
...
const RecipeCard = React.forwardRef(({ recipe, onViewDetails, onSave, isSaved }, ref) => {
  return (
    <div
      ref={ref}
    ...
    >
      <div className="relative">
        <img src={recipe.thumbnail_url} alt={recipe.name} className="w-full h-56 object-cover" />
        <button
          onClick={(e) => {
            e.stopPropagation()
            onSave()
        ...
        >
          <svg
            ...
            viewBox="0 0 24 24"
            stroke="currentColor"
            className={`w-5 h-5 ${isSaved ? "text-blue-500" : "text-gray-500 dark:text-gray-400"}`}
          >
            <path
              strokeLinecap="round"
      ...
      <div className="p-6">
        <h2 className="text-2xl font-semibold mb-2 text-gray-800 dark:text-white">{recipe.name}</h2>
...
function RecipeModal({ recipe, onClose, cookingTips, similarRecipes, onSave, isSaved }) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-8">
          ...
          <img src={recipe.thumbnail_url} alt={recipe.name} className="w-full h-64 object-cover rounded-lg mb-6" />
          <p className="mb-6 text-gray-600 dark:text-gray-300">{recipe.description}</p>
          <div className="mb-6">
          ...
          {similarRecipes.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold mb-2 text-gray-800 dark:text-white">Similar Recipes</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {similarRecipes.slice(0, 3).map((similarRecipe) => (
                  <div key={similarRecipe.id} className="bg-white dark:bg-gray-700 rounded-lg overflow-hidden shadow-md">
                    <img
                    ...
                    />
                    <div className="p-2">
                      <h4 className="text-sm font-medium text-gray-800 dark:text-white">{similarRecipe.name}</h4>
                    </div>
                  </div>
                ))}
              </div>
